...
class GrugTranslator {
  ...
  async translateText(text, tabId) {
    if (!this.apiKey) {
      throw new Error("grug need api key first");
    }
    ...
    if (this.isRateLimited(tabId)) {
      throw new Error("grug translate too fast, must wait");
    }

    const prompt = this.buildGrugPrompt(text);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`grug api angry: ${response.status}`);
      }
  ...
  buildGrugPrompt(text) {
    return `Transform this text into grug-brained style. Keep the core meaning but make it sound like grug is discussing the actual topic. Don't add extra explanations, just translate the style.
   =====
    Exemple grug-brained style text (grug-brained developer in this case):
...
IMPORTANT RULES:
1. If you see <<GRUG_DELIMITER_DO_NOT_TRANSLATE>>, preserve it EXACTLY as is - do not translate or modify this delimiter.
2. Maintain the exact structure of the text, including any line breaks and delimiters.
3. Return only the translated content without adding extra whitespace.
...
Grug translation:`;
  ...
  isRateLimited(tabId) {
    const lastRequest = this.rateLimiter.get(tabId);
    if (!lastRequest) return false;

    const timeDiff = Date.now() - lastRequest;
    return timeDiff < 1000; // 1 second between requests per tab
  }

  updateRateLimit(tabId) {
    this.rateLimiter.set(tabId, Date.now());
  }
...
const translator = new GrugTranslator();
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "checkContentScript") {
    // This will be handled by content.js if it's loaded
    sendResponse({ success: false, error: "Content script not loaded" });
    return false;
  }

  if (request.action === "toggleGrugMode") {
    // First try to send to content script
    chrome.tabs
      .sendMessage(sender.tab.id, { action: "toggleGrugMode" })
      .then((response) => {
    ...
    (async () => {
      try {
        ...
        while (retries < maxRetries) {
          try {
            const checkResponse = await chrome.tabs.sendMessage(sender.tab.id, {
        ...
        // Then send toggle command
        const response = await chrome.tabs.sendMessage(sender.tab.id, {
          action: "toggleGrugMode",
        });
        sendResponse(response);
      } catch (error) {
        console.error("Inject and toggle error:", error);
        sendResponse({ success: false, error: error.message });
      }
    })();
    return true;
  ...
  if (request.action === "getState") {
    ...
    chrome.tabs
      .sendMessage(sender.tab.id, { action: "getState" })
      .then((response) => {
        sendResponse(response);
      })
      .catch(() => {
        // Content script not loaded or error, assume not in grug mode
        sendResponse({ isGrugMode: false });
      });
    return true;
  }
});
